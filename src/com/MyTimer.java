 ...
 * versatile replacement for the {@code Timer}/{@code MyTimerTask}
 * combination, as it allows multiple service threads, accepts various
 * time units, and doesn't require subclassing {@code MyTimerTask} (just
...
public class MyTimer {
     ...
     * @throws NullPointerException if {@code task} or {@code firstTime} is null
     */
    public void scheduleAtFixedRate(MyTimerTask task, Date firstTime,
                                    long period) {
        if (period <= 0)
            throw new IllegalArgumentException("Non-positive period.");
        sched(task, firstTime.getTime(), period);
     ...
     public int purge() {
         ...
         synchronized(queue) {
             for (int i = queue.size(); i > 0; i--) {
                 if (queue.get(i).state == MyTimerTask.CANCELLED) {
                     queue.quickRemove(i);
                     result++;
                 }
             }
...
/**
 * This class represents a timer task queue: a priority queue of MyTimerTasks,
...
class TaskQueue {
     ...
     * ordered on the nextExecutionTime field: The MyTimerTask with the lowest
